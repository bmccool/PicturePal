...
class IPTCInfo(object):
    ...
    def exportXML(self, basetag, extra, filename):
        ...
        for k, v in list((isinstance(extra, dict)
                and [extra] or [{}])[0].items()):
    ...
    def exportSQL(self, tablename, mappings, extra):
        ...
        values = ', '.join(map(E, list(extra.values())
            + [self.data[k] for k in list(mappings.keys())]))
        # process our data
    ...
    def jpegNextMarker(self, fh):  # OK
        ...
        LOG.debug("JpegNextMarker: at marker %02X (%d)", byte, byte)
        return byte
                ...
                if record == 1 and dataset == 90:
                    ...
                    try:
                        ...
                        try:
                            ...
                            LOG.warn('WARNING: problems with charset '
                                    'recognition (%r)', temp)
                            cs = None
    ...
    def collectIIMInfo(self, fh):  # OK
        ...
        while 1:
            ...
            if self.inp_charset:
                try:
                    value = str(value, encoding=self.inp_charset,
                        errors='strict')
                except:
    ...
    def jpegCollectFileParts(self, fh, discardAppParts=False):
        ...
        while 1:
            ...
            # ourselves.
            if (discardAppParts and marker >= 0xe0
                    and marker <= 0xef):
    ...
    def _enc(self, text):
        ...
        if isinstance(text, str):
            ...
            try:
                ...
                res = str(text, encoding=self.inp_charset,
    ...
    def hexDump(dump):
        ...
        length = len(dump)
        P = lambda z: str(((z >= 0x21 and z <= 0x7e) and [z] or ['.'])[0])
        ROWLEN = 18
        res = ['\n']
